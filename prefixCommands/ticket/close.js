    ...
    async execute(message, args, client) {
        ...
        try {
            ...
            if (processingMsg.editable) {
                await processingMsg.edit({ content: '✅ Ticket closed, transcript saved, trader points awarded. Please select a final action in the panel below.' }).catch(console.error);
            }

        } catch (error) {
            console.error('Error closing ticket via command:', error);
            if (processingMsg.editable) { await processingMsg.edit({ content: '❌ There was an error closing the ticket. Please check the console.' }).catch(console.error); }
        }
    },
};