        ...
        try {
            ...
            if (hasMuteRole) {
                if (!canManageRoles) {
                    ...
                    message.channel.send(`⚠️ I don't have permission to remove the mute role. Please check my 'Manage Roles' permission and role hierarchy. Timeout removal will still be attempted if applicable.`).catch(console.error);
                    ...
                    message.channel.send(`⚠️ I cannot remove the mute role "${roleToRemove.name}" due to role hierarchy. Timeout removal will still be attempted if applicable.`).catch(console.error);
                } else {
            ...
            if (isCurrentlyTimedOut) {
                if (!canTimeoutMembers) {
                    ...
                    if (actionsTaken.length === 0 || !hasMuteRole) { 
                        message.channel.send(`⚠️ I don't have permission to remove timeouts. Please check my 'Timeout Members' permission.`).catch(console.error);
                    }
