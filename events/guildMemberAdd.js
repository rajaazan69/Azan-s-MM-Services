                ...
                if (roleToAssign) {
                    try {
                        ...
                        if (error.code === 50013) {
                             ...
                             sendServerModLog( client, '⚠️ Autorole Failed', `Failed to assign role **${roleToAssign.name}** (<@&${roleToAssign.id}>) to ${member.user.tag} (<@${member.id}>) upon joining.`, '#E74C3C', client.user, member.user, null, `Error: ${error.message}. Check bot permissions and role hierarchy.`);
                        }
                    }
                } else {
            ...
            for (const roleId of autoRoleIDs) {
                ...
                if (role) {
                    try {
                        ...
                        if (error.code === 50013) {
                            sendServerModLog(client, '⚠️ Autorole Failed', `Failed to assign role **${role.name}** (<@&${role.id}>) to ${member.user.tag} (<@${member.id}>) upon joining.`, '#E74C3C', client.user, member.user, null, `Error: ${error.message}. Check bot permissions and role hierarchy.`);
                        }
                    }
