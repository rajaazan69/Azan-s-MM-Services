  ...
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (commandName === 'tagcreate') {
        ...
        const { error } = await supabase.from('tags').upsert({ name, message });
        if (error) return interaction.reply({ content: '❌ Failed to create tag.', ephemeral: true });
        return interaction.reply({ content: `✅ Tag \`${name}\` saved.`, ephemeral: true });
      }

      if (commandName === 'tag') {
        const name = options.getString('name');
        const { data, error } = await supabase.from('tags').select('message').eq('name', name).single();
        if (error || !data) return interaction.reply({ content: `❌ Tag \`${name}\` not found.` });
        return interaction.reply({ content: data.message.slice(0, 2000) });
      }

      if (commandName === 'tagdelete') {
        const name = options.getString('name');
        const { error } = await supabase.from('tags').delete().eq('name', name);
        if (error) return interaction.reply({ content: `❌ Failed to delete tag \`${name}\`.`, ephemeral: true });
        return interaction.reply({ content: `🗑️ Tag \`${name}\` deleted.`, ephemeral: true });
      }

      if (commandName === 'taglist') {
        const { data, error } = await supabase.from('tags').select('name');
        if (error || !data.length) return interaction.reply({ content: 'No tags found.' });
        return interaction.reply({ content: data.map(r => `• \`${r.name}\``).join('\n') });
      }

      if (commandName === 'transcript') {
        const parentId = channel.parentId || channel.parent?.id;
        if (parentId !== TICKET_CATEGORY) {
          return interaction.reply({ content: '❌ You can only generate transcripts in ticket channels!', ephemeral: true });
        }
    ...
    if (interaction.isButton()) {
      if (interaction.customId === 'openTicket') {
        await interaction.deferUpdate().catch(() => {}); // ✅ Fix interaction failed
