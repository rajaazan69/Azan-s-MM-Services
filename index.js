...
client.on('interactionCreate', async interaction => {
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (commandName === 'tag') {
        ...
        db.get('SELECT message FROM tags WHERE name = ?', [name], (err, row) => {
          ...
          if (row) interaction.reply({ content: row.message.slice(0, 2000) || '✅ Sent.' });
      ...
      if (commandName === 'delete') {
        ...
        if (parentId === TICKET_CATEGORY) {
          ...
          await interaction.reply({ content: '❌ You can only delete ticket channels!', ephemeral: true }).catch(() => {});
        }
      }
      ...
      if (commandName === 'rename') {
        const parentId = channel.parentId || channel.parent?.id;
        if (parentId !== TICKET_CATEGORY) {
          return interaction.reply({ content: '❌ You can only rename ticket channels!', ephemeral: true });
        }
      ...
      if (commandName === 'add') {
        const parentId = channel.parentId || channel.parent?.id;
        if (parentId !== TICKET_CATEGORY) {
          return interaction.reply({ content: '❌ You can only add users in ticket channels!', ephemeral: true });
        }
      ...
      if (commandName === 'remove') {
        const parentId = channel.parentId || channel.parent?.id;
        if (parentId !== TICKET_CATEGORY) {
          return interaction.reply({ content: '❌ You can only remove users in ticket channels!', ephemeral: true });
        }
        const user = options.getUser('user');
    ...
    if (interaction.isButton() && interaction.customId === 'delete') {
      ...
      if (parentId !== TICKET_CATEGORY) {
        return interaction.reply({ content: '❌ You can only delete ticket channels!', ephemeral: true });
      }
