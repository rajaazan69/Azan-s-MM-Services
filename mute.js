        ...
        try {
            ...
            if (applyTimeout && canTimeout) {
                if (!targetMember.moderatable) {
                     message.channel.send(`⚠️ Could not apply timeout to ${targetMember.user.tag}: User not moderatable. Role mute will still be attempted.`).catch(console.error);
                    ...
                    try {
                        ...
                        message.channel.send(`⚠️ Failed to apply timeout: ${timeoutError.message}. Attempting role mute.`).catch(console.error);
                    }
                }
            } else if (applyTimeout && !canTimeout) {
                 message.channel.send(`⚠️ I lack permission to apply timeouts. Attempting role mute if configured.`).catch(console.error);
            }
            ...
            if (muteRoleId && roleToApplyAsMute && canManageRoles) {
                    ...
                    if (!targetMember.roles.cache.has(roleToApplyAsMute.id)) {
                        try {
                            ...
                            console.error(`[MuteCmd] Failed to apply mute role to ${targetMember.user.tag}:`, roleError.message);
                             message.channel.send(`⚠️ Failed to apply mute role: ${roleError.message}.`).catch(console.error);
                        }
                    } else {
                     ...
                     message.channel.send(`⚠️ Could not apply mute role due to hierarchy. Timeout may have been applied if requested.`).catch(console.error);
                }
                ...
                message.channel.send(`⚠️ Configured mute role not found. Timeout may have been applied if requested.`).catch(console.error);
